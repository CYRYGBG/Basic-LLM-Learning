...
def train_with_grpo(model, tokenizer, train_data, num_iterations=1, num_steps=500, batch_size=4,
    ...
    reward_window = deque(maxlen=sliding_window_size)
    ...
    for iteration in range(num_iterations):
        ...
        for step in range(num_steps):
            ...
            for grpo_iter in range(mu):
                ...
                # 检查点保存
                old_rewards = list(reward_window)  # 深拷贝当前窗口状态
                mean_old = np.mean(old_rewards) if old_rewards else 0.0
                reward_window.append(avg_reward)
                mean_new = np.mean(reward_window)

                if mean_new > mean_old:
                    # 添加全局步数避免覆盖
                    global_step = iteration * num_steps + step
                    tmp_path = os.path.join(
                        "/home/yeqi3/cyr/code/Basic-LLM-Learning/Code/LLM05/saves",
                        f"checkpoint_win{sliding_window_size}_avg{mean_new:.2f}_step{global_step}"
                    )
                    model.module.save_pretrained(tmp_path)
                    tokenizer.save_pretrained(tmp_path)
                    print(f"检测到奖励窗口平均提升: {mean_old:.2f} → {mean_new:.2f}")
                    # 3. 维护队列并删除旧检查点
                    if len(checkpoint_queue) == checkpoint_queue.maxlen:
                        # 弹出最旧的检查点路径
                        oldest_dir = checkpoint_queue.popleft()
                        # 安全删除旧目录
                        if os.path.exists(oldest_dir):
                            try:
                                shutil.rmtree(oldest_dir)
                                print(f"已删除旧检查点: {oldest_dir}")
                            except Exception as e:
                                print(f"删除 {oldest_dir} 失败: {str(e)}")
                    
                    # 将新路径加入队列
                    checkpoint_queue.append(tmp_path)
