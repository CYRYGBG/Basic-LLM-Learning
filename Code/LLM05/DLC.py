...
# 修改后的训练函数
def ddp_train_with_grpo(
    model, tokenizer, train_data, sampler, 
    num_iterations=1, num_steps=500, batch_size=4,
    num_generations=4, max_completion_length=128, beta=0.1,
    learning_rate=5e-6, mu=3, epsilon=0.2, 
    reward_function=None, device_ids=None
):
    # 模型移动到当前设备
    ...
    for iteration in range(num_iterations):
        sampler.set_epoch(iteration)  # 设置epoch以便正确shuffle
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            sampler=sampler,
            pin_memory=True,
            collate_fn=lambda x: x  # 保持原有数据格式
        )
        
        # 后续训练逻辑保持不变，使用train_loader迭代数据
        for step, batch_samples in enumerate(train_loader):
            # 原始的数据处理代码保持不变
            # generate_rollout_data 等函数保持原样
            # grpo_loss 计算保持原样
            # 参数更新保持原样
# 模型初始化（保持原有逻辑）
...
# 执行分布式训练
ddp_train_with_grpo(
    model, tokenizer, train_data, train_sampler,
    **training_config
)
...
def train_with_grpo(model, tokenizer, train_data, num_iterations=1, num_steps=500, batch_size=4,
    ...
    # 配置tensorboard 
    writer = SummaryWriter(log_dir="runs/AllData")  
